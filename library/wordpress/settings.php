<?php
class wv21v_settings extends bv21v_settings {
	...
	public function saved_data_locations($add_default=true)
	{
		if (is_null ( $this->saved_data_locations )) {
			...
			$type = $type['base'];
			$options = new wv21v_table_options ($type);
			...
			$table = new wv21v_table (null, strtolower ( $this->settings ()->application['name'] ));
	...
	public function all()
	{
		if(null===$this->_settings)
		{
			...
			if(is_array($options))
			{
				$this->_settings=bv21v_type_array::merge_replace_recursive($this->_settings,$options);
			}
	...
	protected function legacy_move($old,$section=null,$new=null)
	{
		...
		if (is_array ( $old_data )) {
			...
			if(!empty($new_data))
			{
				$new_data = bv21v_type_array::merge_replace_recursive($new_data,$old_data);
			}
			else
			{
				$new_data=$old_data;
