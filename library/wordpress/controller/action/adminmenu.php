<?php
abstract class wv21v_controller_action_adminmenu extends wv21v_controller_action_abstract {
	...
	protected function read_notice() {
		$table = new wv21v_table_options ( 'DCoda_Notices' );
		$options = $table->get ();
	...
	public function setup() {
		...
		global $menu;
		$mnu = $this->find_menu($cm['menu']);
		//if ($cm ['title'] == $cm ['menu']) {
		$page_title =  $cm ['menu'];
		$menu_title = $cm['title'];
		$capability = 'administrator';
		$function = array ($this, 'controller' );
		$menu_slug = get_class($this);
		if(null===$mnu)
		{
			/*
			 * positions in menu
