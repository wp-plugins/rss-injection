<?php
class wv25v_table_blogs extends wv25v_table {
	...
	public function move($blog_id, $site_id) {
		$wv25v_table_sites = new wv25v_table_sites ( );
		if (! $this->exists ( $blog_id )) {
			return false;
		}
		if (! $wv25v_table_sites->exists ( $site_id )) {
			return false;
		}
		$site = $wv25v_table_sites->get ( $site_id );
		...
		$wp_options = new wv25v_table_options ( $blog_id );
