<?php
class wv21v_table_blogs extends wv21v_table {
	...
	public function move($blog_id, $site_id) {
		$wv21v_table_sites = new wv21v_table_sites ( );
		if (! $this->exists ( $blog_id )) {
			return false;
		}
		if (! $wv21v_table_sites->exists ( $site_id )) {
			return false;
		}
		$site = $wv21v_table_sites->get ( $site_id );
		...
		$wp_options = new wv21v_table_options ( $blog_id );
