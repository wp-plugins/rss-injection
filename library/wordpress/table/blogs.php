<?php
class wv26v_table_blogs extends wv26v_table {
	...
	public function move($blog_id, $site_id) {
		$wv26v_table_sites = new wv26v_table_sites ( );
		if (! $this->exists ( $blog_id )) {
			return false;
		}
		if (! $wv26v_table_sites->exists ( $site_id )) {
			return false;
		}
		$site = $wv26v_table_sites->get ( $site_id );
		...
		$wp_options = new wv26v_table_options ( $blog_id );
