<?php
class wv25v_table_sites extends wv25v_table {
	...
	public function add($domain, $path = '/', $create_blog = true) {
		...
		if (! $this->exists_by_domain ( $domain, $path )) {
			...
			if ($site_id) {
				if ($create_blog) {
					$wv25v_table_blogs = new wv25v_table_blogs ( );
					$blog_id = $wv25v_table_blogs->add ( $site_id );
				}
				...
				$prime_site_meta = new wv25v_table_sitemeta ( 1 );
				$new_site_meta = new wv25v_table_sitemeta ( $site_id );
