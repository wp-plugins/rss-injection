<?php
class wv26v_table_sites extends wv26v_table {
	...
	public function add($domain, $path = '/', $create_blog = true) {
		...
		if (! $this->exists_by_domain ( $domain, $path )) {
			...
			if ($site_id) {
				if ($create_blog) {
					$wv26v_table_blogs = new wv26v_table_blogs ( );
					$blog_id = $wv26v_table_blogs->add ( $site_id );
				}
				...
				$prime_site_meta = new wv26v_table_sitemeta ( 1 );
				$new_site_meta = new wv26v_table_sitemeta ( $site_id );
