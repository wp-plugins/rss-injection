 ...
 * $LastChangedBy$
 */
class SandBoxSandBoxController extends DCode_Controller_Action_WP_AdminMenu
{
	public function FilesAction ( $content )
	{
		...
		$external = 0;
		$project = 0;
		$pclass = new Project ($this->application());
		$phpdoc = new Project ( $this->application () , $pclass->plugin () );
		$plugin = $phpdoc->blocks ();
		$plugin = $plugin [0];
		$phpdoc = new Project ( $this->application () , $pclass->readme () );
		...
		foreach ( $pclass->files () as $file )
		{
			...
			{
				$phpdoc = new Project ( $this->application () , $file ['file'] );
				$info = $phpdoc->blocks ();
				$error = array ();
				if (count ( $info ) > 0)
				{
	...
	protected function plugins ()
	{
		...
		{
			$projectsDir = new DCode_FS ( $this->application () , dirname ( dirname ( dirname ( dirname ( dirname ( __FILE__ ) ) ) ) ) . '/' . $dir . '/' );
			$php = $projectsDir->dir ( "*.php" , DCode_FS::type_file , 1 );
			...
			foreach ( $php as $file )
			{
				$p = new DCode_Application_Info ( $this->application () , $file );
				if ($p->author () == "DCoda Ltd")
				{
					$pmore ['info'] = $p;
					$pmore ['svn'] = new DCode_Application_Info_SVN ( $this->application () , $file );
					$pmore ['readme'] = new DCode_Application_info_Readme ( $this->application () , $file );
					$this->plugins [] = $pmore;
				}
			}
		}
	...
	{
		$mysql = new WP_Users ( );
		DCode_Debug::show ( $mysql->key_fields () );
	}

	//--- test
	public function testAction ()
	{
		$test = new DCode_Gzip ( $this->application () );
		$test->make_file ();
		DCode_Debug::show ( '' );
	}
}
