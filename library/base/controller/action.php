<?php
abstract class bv25v_controller_action extends bv25v_base {
	protected function marker($tag, $content) {
		$tagc = bv25v_tag::instance ();
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv25v_view ( $this->application () );
		}
	}
...
//		$this->trace();
		...
		if ($string === false) {
			...
			if ($script !== false) {
				switch ($pi ['extension']) {
					case 'xml' :
						$data = new bv25v_data_xml ( $this->application (), $script, true );
						$return = $data->load ();
						$html = false;
						break;
					default :
