<?php
abstract class bv26v_controller_action extends bv26v_base {
	protected function marker($tag, $content) {
		$tagc = bv26v_tag::instance ();
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv26v_view ( $this->application () );
		}
	}
...
//		$this->trace();
		...
		if ($string === false) {
			...
			if ($script !== false) {
				switch ($pi ['extension']) {
					case 'xml' :
						$data = new bv26v_data_xml ( $this->application (), $script, true );
						$return = $data->load ();
						$html = false;
						break;
					default :
