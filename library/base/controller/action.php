<?php
abstract class bv21v_controller_action extends bv21v_base {
	protected function marker($tag, $content) {
		$tagc = bv21v_tag::instance ();
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv21v_view ( $this->application () );
		}
	}
	...
	protected function render_script($script, $html = true) {
		...
		if ($string === false) {
			...
			if ($script !== false) {
				switch ($pi ['extension']) {
					case 'xml' :
						$data = new bv21v_data_xml ( $this->application (), $script, true );
						$return = $data->load ();
						$html = false;
						break;
					default :
