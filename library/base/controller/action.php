<?php
abstract class bv27v_controller_action extends bv27v_base {
	protected function marker($tag, $content) {
		$tagc = bv27v_tag::instance ();
	...
	protected function set_view() {
		if (null === $this->view) {
			$this->view = new bv27v_view ( $this->application () );
		}
	}
...
//		$this->trace();
		...
		if ($string === false) {
			...
			if ($script !== false) {
				switch ($pi ['extension']) {
					case 'xml' :
						$data = new bv27v_data_xml ( $this->application (), $script, true );
						$return = $data->load ();
						$html = false;
						break;
					default :
