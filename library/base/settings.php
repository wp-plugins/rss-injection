<?php
class bv25v_settings extends bv25v_controller_action {
	...
	public function all()
	{
		if(null===$this->_settings)
		{
			...
			foreach($array as $setting)
			{
				...
				$combined_settings=bv25v_type_array::merge_replace_recursive($combined_settings,$xml);
			}
			if(isset($combined_settings['xml']))
			{
				...
				{
					$xml = $this->render_script($dir.$setting);
					$combined_settings=bv25v_type_array::merge_replace_recursive($xml,$combined_settings);
				}
			}
			if(null!==$this->alternate() && isset($combined_settings['alternate'][$this->alternate()]))
			{
				$combined_settings=bv25v_type_array::merge_replace_recursive($combined_settings,$combined_settings['alternate'][$this->alternate()]);
			}
