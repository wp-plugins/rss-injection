<?php
class bv27v_settings extends bv27v_controller_action {
	...
	public function all()
	{
		if(null===$this->_settings)
		{
			...
			foreach($array as $setting)
			{
				...
				$combined_settings=bv27v_type_array::merge_replace_recursive($combined_settings,$xml);
			}
			if(isset($combined_settings['xml']))
			{
				...
				{
					$xml = $this->render_script($dir.$setting);
					$combined_settings=bv27v_type_array::merge_replace_recursive($xml,$combined_settings);
				}
			}
			if(null!==$this->alternate() && isset($combined_settings['alternate'][$this->alternate()]))
			{
				$combined_settings=bv27v_type_array::merge_replace_recursive($combined_settings,$combined_settings['alternate'][$this->alternate()]);
			}
	...
	public function config_files() {
		...
		$this->view->version .= '.'.str_replace('v','','v27v').$version;
		$this->view->name = str_replace('.','',$version_name).'&nbsp;'.$this->view->name;
		// add the library version to the main version. replace the vs with place. replace is used so v27v will match to serach and replace
		$this->view->handler='';
