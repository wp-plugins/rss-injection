 ...
 * $LastChangedBy$
 */
abstract class DCode_Controller_Action_WP_Abstract extends DCode_Controller_Action
{
	...
	{
		$this->view = new DCode_WP_View ( $this->application () );
	}
	...
	protected function selected_action_wp()
	{
		...
		if (count ( $filter ) > 1 && $filter [1] == 'page')
		{
			...
			else
			{
				foreach ( $this->actions () as $r )
				{
					if ($pages ['page2'] == DCode_Type_String::staticSafe ( $r ['title'] ))
					{return $r;}
				}
			}
		}
	...
	protected function subpages ()
	{
		...
		{
			if (DCode_Type_String::staticStartsWith ( $key , 'page' ))
			{
				$pages [$key] = $value;
			}
		}
