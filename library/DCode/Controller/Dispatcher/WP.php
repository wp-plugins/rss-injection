 ...
 * $LastChangedBy$
 */
class DCode_Controller_Dispatcher_WP extends DCode_Controller_Dispatcher_Standard
{
	...
	public function controllers ()
	{
		...
		foreach ( $dirs as $dir )
		{
			$fs = new DCode_FS ( $this->application () , $dir );
	...
	public function setup ( $notmenu = false )
	{
		...
		foreach ( $this->controllers () as $controller )
		{
			...
			if ($notmenu)
			{
				...
				{
					case DCode_Controller_Action_WP_Abstract::WP_FILTER :
					case DCode_Controller_Action_WP_Abstract::WP_ACTION :
					case DCode_Controller_Action_WP_Abstract::WP_CONTROL :
						$controllerClass->setup ();
				}
			}
			else
			{
				if ($controllerClass->getType () == DCode_Controller_Action_WP_Abstract::WP_DASHBOARD)
				{
					$controllerClass->setup ();
				}
			}
