<?php
/**
 * Routines to pull or push data using tcpip
 * @package Library
 * @subpackage Http
 * @copyright DCoda Ltd
 * @author DCoda Ltd
 ...
 * $LastChangedBy$
 */
class DCode_Http extends DCode_Base
{
	private $Timeout = 60;
	...
	{
		parent::__construct();
		$this->user = new DCode_Value();
		$this->password = new DCode_Value();
		$this->url = $url;
		$this->dummyHeaders();
	}
	public function get ()
	...
	private function dummyHeaders ()
	{
		...
		foreach ($_SERVER as $key => $value) {
			if (DCode_StringFunctions::startsWith($key, 'http_')) {
	...
	private function request ($Url, $Method = 'GET', $Data = null)
	{
		...
		if ($Method == 'POST') {
			...
			$this->addHeader('Content-Lengtht', strlen($Data));
		}
